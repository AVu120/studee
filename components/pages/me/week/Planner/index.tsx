...
export const Planner = ({ weeklyPlan, setWeeklyPlanState }: Props) => {
  const onChangeNotes = (e: ChangeEvent<HTMLTextAreaElement>) => {
    ...
    >
      ...
      {[1, 2, 3, 4, 5, 6, 0].map((num, i) => {
        ...
        const [startDateYear, startDateMonth, startDateDay] =
          startDateParams.map((numString) => Number(numString));
      ...
      <Notes onChange={onChangeNotes} value={weeklyPlan.notes || ""} />
    </Grid>
  );
};
